...
{
public:
    ZephyrWifiScanResponseIterator(const size_t size, const struct wifi_scan_result * scanResults) : mSize(size), mpScanResults(scanResults) {}
    ...
    bool Next(WiFiScanResponse & item) override
    {
        ...
        item.ssidLen = mpScanResults[mIternum].ssid_length;
        ...
        memcpy(item.bssid, mpScanResults[mIternum].mac, 6);
...
private:
    const size_t mSize;
    const struct wifi_scan_result * mpScanResults;
    size_t mIternum = 0;
};
...
public:
    ...
    void OnScanWiFiNetworkDone(const struct wifi_scan_result *result, uint32_t num_result);
    void OnNetworkStatusChange();

    CHIP_ERROR SetLastDisconnectReason(const ChipDeviceEvent * event);
    int32_t GetLastDisconnectReason();

    static ZephyrWiFiDriver & GetInstance()
    {
